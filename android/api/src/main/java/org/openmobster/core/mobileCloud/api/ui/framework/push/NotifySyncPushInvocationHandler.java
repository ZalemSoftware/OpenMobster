...
{
	public NotifySyncPushInvocationHandler()
	{
	...
	private void notify(Context context, String channel,List<MobileBeanMetaData> beanMetaData) throws Exception
	{
		 ...
		 * Isto se deve ao fato de que nas versões mais recentes do Android (API 19) o "PendingIntent.getActivity" lança NullPointerException
		 * se o "intent" for null. Aparentemente este erro não acontecia na versão do Android em que o OpenMobster usava (API 11).
		 * O erro já foi identificado aqui: https://code.google.com/p/android/issues/detail?id=23871
		 * 
		 * Esta modificação também evita com que uma simples atualização de dados vindo do servidor dispare uma notificação sonora.
		 * Só será liberada a notificação se foi definida uma Activity para tratar os pushes.
		 */
