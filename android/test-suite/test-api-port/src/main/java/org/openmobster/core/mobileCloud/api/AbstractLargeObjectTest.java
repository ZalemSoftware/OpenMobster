 ...
 *
 */
public abstract class AbstractLargeObjectTest extends Test 
{
	protected final String syncService = "sync://SyncService";
	protected final String service = "large_object_channel";
	
	public void setUp() 
	{		 
		MobileObjectDatabase.getInstance().deleteAll(service);
		this.resetServerAdapter("setUp="+this.getClass().getName()+"/SetUpLargeObjectTestSuite\n");
	}
	...
	protected void startTwoWaySync() throws Exception
	{		
		SyncInvocation syncInvocation = new SyncInvocation(
		"org.openmobster.core.mobileCloud.android.invocation.SyncInvocationHandler", 
		SyncInvocation.twoWay, this.service);		
		Bus.getInstance().invokeService(syncInvocation);		
	}
