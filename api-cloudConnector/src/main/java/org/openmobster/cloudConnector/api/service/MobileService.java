...
package org.openmobster.cloudConnector.api.service;
...
import org.openmobster.cloudConnector.api.Tools;
...
public final class MobileService 
{
	...
	public static Response invoke(Request request)
	{
		...
		try
		{
			...
			log.info("Handshake Status---------------------------------------------------------");	
			log.info(response);			
			log.info("-------------------------------------------------------------------------");	
			if(response.contains("status=401"))
			{
				//request was considered unauthorized
				return null;
			}
